...
Survival profiles are exported as cumulative percent curves reconstructed from Module 4 annual survival probabilities. Vintage profiles are exported as percent age distributions that sum to 100. LEAP requires stock-vintage profiles to start with zero vehicles at age zero because base-year stock excludes vehicles sold in the base year. The exporter therefore writes `Year 0 = 0` and shifts every model vintage-share age forward by one year; it does not discard or alter the underlying distribution. The workbook also includes a `Constant` degradation profile with a value of 100 at every vehicle age for Mileage and Fuel Economy. Each worksheet uses a clean Excel identifier such as `Freight_vehicle_survival`, and the workbook defines a range with the same name over that sheet's value cells (`B5` through the final profile row). In LEAP, use the worksheet name for both the new lifecycle profile name and its Excel Range. The output directory includes the combined workbook, a `lifecycle_profile_manifest.csv`, and a downloadable `<economy>_lifecycle_profiles.zip`.
