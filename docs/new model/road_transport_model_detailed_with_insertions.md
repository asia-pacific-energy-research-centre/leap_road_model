...
- `Passenger Stock Growth Rate Adjustment` for Module 3 passenger GDP-per-capita income-elasticity tuning;
...
- reconciliation weights for Module 6 (required â€” Module 1 provides APEC-wide defaults that can be overridden per economy).
...
Temporary scrappage policies are currently represented in Module 4 as explicit
year-specific `scrappage_for_leap` values and can be used by the Python mirror.
They are not currently emitted by `build_leap_ready_table()` as routine T11
`Scrappage` rows. If scrappage becomes a standard LEAP scenario lever, it should
be exported as explicit year-specific scrappage assumptions rather than baked
into a permanent survival curve unless the policy is meant to represent a
...
- final stock after the cohort adjustment; and
- explicit `scrappage_for_leap` values when supplied.

Module 4 also produces `T6v_vintage_profiles`, the age-profile table used by
the lifecycle exporter. This is where vintage shares, survival probabilities,
and any age-shift marker are written. The workflow writes LEAP-compatible
lifecycle profile workbooks from the final active `T6v` under
...
```text
energy_pj = stock Ã— mileage / efficiency_km_per_gj / 1,000,000
```
...
```text
remaining_esto_gasoline = ESTO_gasoline âˆ’ PHEV_gasoline
```
...
#### Biofuel allocation and Device Share interpretation risk

Biodiesel and biogasoline are currently treated as separate observed ESTO fuel streams and allocated to eligible road branches. This is useful for preserving the fuel balance, but it creates an interpretation risk. If reported biofuel energy is large relative to the eligible modelled demand, the resulting Device Shares can make it look as though there is a large dedicated biofuel vehicle fleet.

That is not necessarily the intended real-world interpretation. In many cases, biofuels are blended into conventional fuels rather than consumed by a separate set of dedicated biofuel vehicles. Users should therefore interpret high biodiesel or biogasoline Device Shares as a fuel-balance allocation outcome unless there is specific evidence that the economy has a dedicated biofuel vehicle stock. Dashboard outputs should flag this where needed so that biofuel shares are not mistaken for technology penetration.
...
efficiency_scalar = ECF ^ âˆ’0.25     â† inverted: higher efficiency reduces energy
```
...
```text
adjusted_stock      = stock      Ã— stock_scalar
adjusted_mileage    = mileage    Ã— mileage_scalar
adjusted_efficiency = efficiency Ã— efficiency_scalar
```
...
```text
final_energy_pj = adjusted_stock Ã— adjusted_mileage / adjusted_efficiency / 1,000,000
```
...
every branch Ã— variable Ã— scenario combination. The ID columns must match LEAP's
...
Demand
  Measures:
    PHEV Electric Driving Share
      Applies at: Demand\PHEV <size>
      Note: special contract row used by Module 6 for PHEV/EREV electric-liquid split.
...
```text
Demand-level special rows
  Measures:
    PHEV Electric Driving Share
  Used for:
    PHEV/EREV electric-driving utilisation in Module 6.
