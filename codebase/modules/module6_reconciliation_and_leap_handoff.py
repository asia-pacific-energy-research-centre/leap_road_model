...
def build_leap_ready_table(
    ...
    if not sales_shares.empty and "sales_share" in sales_shares.columns:
        ...
        if "size" in vt_dt_path.columns and "size" not in sales_shares.columns and "size" in ss_path.columns:
            ...
            # to an equal split across the number of technology variants. Count
            # distinct sizes/paths rather than rows because sales shares usually
            # span many years.
            grp_keys = [k for k in ["economy", "scenario", "vehicle_type", "drive_type"] if k in ss_path.columns]
            n_sizes = ss_path.groupby(grp_keys, dropna=False)["size"].transform(
                lambda values: values.nunique(dropna=True)
            )
            n_paths = ss_path.groupby(grp_keys, dropna=False)["_tech_path_lookup"].transform(
                lambda values: values.nunique(dropna=True)
            )
            n_sizes = n_sizes.mask(n_sizes <= 0, n_paths)
            n_sizes = n_sizes.replace(0, np.nan)
