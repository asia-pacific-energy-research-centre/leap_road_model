...
def test_russia_registry_run_uses_2022_package_base_year(tmp_path: Path) -> None:
    result = run_for_economy(
    ...
    assert result["workflow_meta"]["base_year"] == 2022
    assert result["workflow_meta"]["base_year_provenance"] == "legacy_inferred"
    assert "legacy_package_rebase" not in result["workflow_meta"]
